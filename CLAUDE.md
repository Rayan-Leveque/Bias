...
├── test5.py              # Visualization: multi-panel catplot from result_implicit.csv
├── 789.py                # Alternative visualization script
...
└── implicit_bias_fixed.{pdf,png}  # Output visualizations
```
...
See `cleaning.py:181` (`d_score()`) for the implementation.

### Stimulus Structure (`iat_stimuli.csv`)
...
**Pipeline stages:**
1. Load and concatenate all CSVs from `results/`
2. Parse raw LLM responses → `"word - group"` format (`format_response()`)
3. Load stimulus labels from `iat_stimuli.csv`
...
**Key functions:**
- `cleaning.py:66` — `format_response()`: multi-strategy parser for LLM output
- `cleaning.py:181` — `d_score()`: IAT bias score calculation
- `cleaning.py:274` — `bootstrap_ci()`: 95% CI via resampling (seed=42, n=10000)
- `cleaning.py:297` — `permutation_test_bias()`: H₀: mean D-score = 0
- `cleaning.py:319` — `compute_statistics()`: grouped stats by model × category × dataset

### `config_loader.py` — Configuration Management
Loads and validates `models_config.yml`. Use it to inspect or extend model definitions.
...
### `weat.py` — Embedding-Based Bias Metrics
...
### `test5.py` — Visualization
Reads `result_implicit.csv` and produces a 4-panel seaborn catplot (one column per model), with bias categories on the x-axis and D-scores on the y-axis. Outputs `implicit_bias_fixed.pdf` and `implicit_bias_fixed.png`.

### `qwe.py` — Ad-hoc API Testing
...
## Configuration (`models_config.yml`)
...
To add or disable a model, edit `models_config.yml` and set `enabled: true/false`.
...
```
iat_stimuli.csv  ──┐
models_config.yml  ├──► [inference script / qwe.py]  ──► results/*.csv
                   │
results/*.csv  ────┴──► cleaning.py  ──► result_implicit.csv
                                              │
                                              ├──► result_implicit_stats.csv
                                              │
                                              └──► test5.py  ──► implicit_bias_fixed.{pdf,png}
```

---

## Raw Result CSV Schema (`results/*.csv`)

Expected columns consumed by `cleaning.py`:
...
After cleaning, `result_implicit.csv` adds: `iat`, `formatted_iat`, `flag`, `iat_bias`.
...
### Language
- Code is Python 3; type hints are used in `cleaning.py` and `config_loader.py`
...
- Verify parsing logic against the existing response format in `results/*.csv`
...
### Extending Stimuli
To add new bias categories, append rows to `iat_stimuli.csv` with the schema:
...
### Adding a New Model
1. Add an entry to the `models:` section of `models_config.yml`
2. Set `enabled: true` and configure `tensor_parallel_size` / `pipeline_parallel_size` for your GPU setup
3. Run inference (producing a CSV in `results/`)
4. Run `cleaning.py` to regenerate `result_implicit.csv`
5. Re-run `test5.py` to update visualizations
...
- `result` — output result files (prefix match)
- `results_temp_08/` — temporary results directory
- `test*` — any test scripts or output files
